...
- `npm run start:dev` - Запуск frontend проекта на webpack dev server + JSON server backend
- `npm run start:dev:vite` - Запуск frontend проекта на vite + JSON server backend
- `npm run start:dev:server` - Запуск JSON server backend сервера
...
- `npm run test:ui` - Запуск скриншотных тестов с loki (нужен Docker)
...
Некоторые запросы на сервер отправляются с помощью [RTK query](/src/shared/api/rtkApi.ts).

Для асинхронного подключения редюсеров (чтобы не тянуть их в общий бандл) используется хук 
[useDynamicReducerLoader](src/shared/lib/hooks/useDynamicReducerLoader/ui/useDynamicReducerLoader.tsx)
